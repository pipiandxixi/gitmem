...
- gitmem
- mcp
updated_at: '2026-01-07T23:29:05.219235'
---
...
## quant_deploy (21:51:40)
<!-- tags: ['deployment', 'config'] -->
QuantServer Deployment Config (Update):
1. Python version must be 3.12 (not 3.11).
2. Default port is 9090.
3. Use 'gunicorn' with 4 workers.
4. Log path: /var/log/qs/access.log

## manual_test (22:20:00)
This is a manual test to verify diff parsing.

## gitmem_overview (23:29:05)
GitMem 是一个利用 Git 作为后端存储的语义化记忆服务，通过 MCP 协议为 AI Agent 提供持久化、可回溯的长期记忆。核心原则包括：摄入重于分类（按日归档）、Git 为本、元数据驱动以及通过消化产生知识。目前已实现基础存储、多租户、存储架构重构和基础 MCP 工具集。未来重点是开发消化服务、语义化重构工具集以及服务端 LLM 集成。